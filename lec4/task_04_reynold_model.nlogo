...
to go
  ...
  if pen-clear? [ clear-drawing ]
  tick
end

to update-heading
  ask birds [
    ; определяем соседей (mates) данной птицы, которые находятся в области ее видимости
    let mates other birds in-cone vision (2 * omega)

    if any? mates [
      set n-mates count mates

      ; веса каждого из трех правил
      ; let alpha 0.2 + random-float 0.2
      ; let beta  0.2 + random-float 0.2
      ; let gamma 0.1 + random-float 0.1

      let alpha _alpha + random-float 0.2
      let beta  _beta  + random-float 0.2
      let gamma _gamma + random-float 0.1

      let h-sep heading   ; направление для уклонения от ближайшей птицы
      let h-align heading ; среднее направление движения птиц в области видимости
      let h-flock heading ; среднее направление ко всем птицам в области видимости
      ...
      if separate? [
        ...
        ; если ближайший сосед ближе чем минимальное допустимое расстояние, то отдаляемся от него
      ...
      ; 2 правило Рейнолдса:
      if align? [
        ; находим покоординатную сумму всех единичных векторов, определяющих направления движения птиц из множества mates
      ...
      ; 3 правило Рейнолдса:
      if flock? [
        ; определяем средние значения направлений движения птиц из множества mates
        let xf sum [sin (towards myself + 180)] of mates
        let yf sum [cos (towards myself + 180)] of mates
        set h-flock atan xf yf
      ]

      ; вычисляем новое расположение птицы
